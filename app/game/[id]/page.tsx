...
export default function Home() {
  ...
  function evaluateHand(playerCards: CardType[], communityCards: CardType[]): number {
    ...
    // straight flush
    const suitsCount: Record<suit, number[]> = { heart: [], spade: [], club: [], diamond: [] }
    allCards.forEach(card => {
      suitsCount[card.suit].push(card.number)
    })
    for (const suitKey in suitsCount) {
      const numbers = suitsCount[suitKey as suit].sort((a, b) => b - a)
